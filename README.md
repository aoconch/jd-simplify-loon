...
- **v3.0 的应对**：把 `pro.m.jd.com` 加入 `[Mitm]`，并对首页网页 HTML 注入脚本：
  1. **锁死推荐流回调**：用 `Object.defineProperty` 把 `getRecommendPageSourceCallback` 锁成空函数，feed 永不渲染；
  2. **内容感知隐藏**：`MutationObserver` + 关键词（猜你喜欢/为你推荐/百亿补贴…）扫描，把命中推荐的楼层容器 `display:none`，滚动加载也能持续生效；
  3. **JSON 接口分支（我的页 / 小接口）**：命中 `uniformRecommend`/`guessYouLike`/`recommend*` 直接返回空数据；`secondFloor` 移除 `recommendFloor`；所有可处理接口递归清理广告字段。
...
```
https://raw.githubusercontent.com/aoconch/jd-simplify-loon/8d12a244ed900d03831510b084866a9a5be199f7/jd-simplify.plugin
```
...
- **`X-JD-WebView: 1`** → 首页 H5 网页已被注入去推荐脚本（v3.0 新增，关键指标）；
- **`X-JD-Simplified: 1`** → 某 JSON 接口已被改写（通用广告清理）；
- **`X-JD-Simplified: 1-feed`** → 某推荐流接口已被清空。

> 若 `X-JD-WebView` 头出现，说明首页网页已真正被处理；若只有 `X-JD-Simplified` 而无 `X-JD-WebView`，说明 MITM 仍未覆盖 `pro.m.jd.com`，需检查第 2 步。
